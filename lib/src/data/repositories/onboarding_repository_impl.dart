...
class OnBoardingRepositoryImpl implements OnBoardingRepository {
  ...
  Future<Either<Failure, List<DistrictModel>>> getDistrict(
      DistrictsParams districtsParams) async {
    try {
      final districts =
  ...
  Future<Either<Failure, Ticket>> getTicket(generateTicketParams) async {
    try {
      final token = await localDataSource.getSessionToken();
      final tickets = await remoteDataSource.postGenerateTicket(
          token, generateTicketParams);
  ...
  Future<Either<Failure, ScheduleResponseModel>> getScheduleForBranch(
    ...
    try {
      ...
      final scheduleResponse =
          await remoteDataSource.getScheduleByBranch(token, params);
